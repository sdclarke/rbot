...
pub fn interpret_start_message(message: &str) -> Result<bool, &'static str> {
    ...
    return Err("Unknown start message format");
}
...
pub fn interpret_state_message(message: &str, board: &mut Board) -> Result<MoveTurn, &'static str> {
    ...
    for i in 0..board.get_no_of_holes() {
        board.set_seeds(North, i+1, board_parts[i].parse::<u8>().unwrap());
    }
    board.set_seeds_in_store(North, board_parts[board.get_no_of_holes()].parse::<u8>().unwrap());
    for i in 0..board.get_no_of_holes() {
        board.set_seeds(South, i+1, board_parts[i+board.get_no_of_holes()+1].parse::<u8>().unwrap());
    }
    board.set_seeds_in_store(South, board_parts[2*board.get_no_of_holes()+1].parse::<u8>().unwrap());
